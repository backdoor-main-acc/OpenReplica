...
from app.events import EventSource, EventStream
...
def display_banner(session_id: str) -> None:
    ...
    print_formatted_text(HTML(f'<grey>OpenReplica CLI v{__version__}</grey>'))
...
def display_event(event: Event, config: OpenReplicaConfig) -> None:
...
def display_help() -> None:
    ...
    print_formatted_text(
        HTML(
            f'\n<grey>OpenReplica CLI v{__version__}</grey>\n'
            '<gold>OpenReplica CLI lets you interact with the OpenReplica agent from the command line.</gold>\n'
        )
    )
    ...
    # Tips section
    print_formatted_text(
        'Some tips to get the most out of OpenReplica:\n'
