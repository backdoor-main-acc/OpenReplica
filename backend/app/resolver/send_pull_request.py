...
from app.resolver.interfaces.github import GithubIssueHandler
from app.resolver.interfaces.gitlab import GitlabIssueHandler
from app.resolver.interfaces.issue import Issue
from app.resolver.interfaces.issue_definitions import ServiceContextIssue
from app.resolver.io_utils import (
    load_single_resolver_output,
)
from app.resolver.patching import apply_diff, parse_patch
from app.resolver.resolver_output import ResolverOutput
from app.resolver.utils import identify_token
from app.utils.async_utils import GENERAL_TIMEOUT, call_async_from_sync
...
def send_pull_request(
    ...
    if additional_message:
        pr_body += f'\n\n{additional_message}'
    pr_body += '\n\nAutomatic fix generated by [OpenReplica](https://github.com/All-Hands-AI/OpenReplica/) 🙌'
...
def update_existing_pull_request(
    ...
    if not comment_message and additional_message:
        try:
            ...
            if explanations:
                comment_message = (
                    'OpenReplica made the following changes to resolve the issues:\n\n'
                )
            ...
            comment_message = f'A new OpenReplica update is available, but failed to parse or summarize the changes:\n{additional_message}'
