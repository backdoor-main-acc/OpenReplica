...
from app.events.nested_event_store import NestedEventStore
from app.events.stream import EventStream
from app.integrations.provider import PROVIDER_TOKEN_TYPE, ProviderHandler
from app.llm.llm import LLM
from app.runtime.impl.docker.containers import stop_all_containers
...
from app.server.session.session import ROOM_KEY, Session
...
from app.storage.locations import get_conversation_dir
...
class DockerNestedConversationManager(ConversationManager):
    ...
    async def _create_runtime(self, sid: str, user_id: str | None, settings: Settings):
        ...
        volumes.append(
            f'{config.file_store_path}/{conversation_dir}:{OpenReplicaConfig.model_fields["file_store_path"].default}/{conversation_dir}:rw'
        )
