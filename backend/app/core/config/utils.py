...
def load_from_env(
    cfg: OpenReplicaConfig, env_or_toml_dict: dict | MutableMapping[str, str]
) -> None:
    ...
    Args:
        cfg: The OpenReplicaConfig object to set attributes on.
        ...
        for field_name, field_info in sub_config.model_fields.items():
            ...
            if isinstance(field_value, BaseModel):
                ...
                try:
                    ...
                    logger.openreplica_logger.error(
...
def load_from_toml(cfg: OpenReplicaConfig, toml_file: str = 'config.toml') -> None:
    """Load the config from the toml file. Supports both styles of config vars.

    Args:
        cfg: The OpenReplicaConfig object to update attributes of.
    ...
    try:
        ...
        logger.openreplica_logger.warning(
    ...
    if 'condenser' in toml_config:
        try:
            ...
            if 'condenser' in condenser_mapping:
                ...
                logger.openreplica_logger.debug(
    ...
    # set LLMSummarizingCondenserConfig as default
        ...
        from app.core.config.condenser_config import LLMSummarizingCondenserConfig
    ...
    for key in toml_config:
        if key.lower() not in known_sections:
            logger.openreplica_logger.warning(f'Unknown section [{key}] in {toml_file}')
...
def finalize_config(cfg: OpenReplicaConfig) -> None:
...
def get_agent_config_arg(
    ...
    logger.openreplica_logger.debug(f'Loading agent config from {agent_config_arg}')
    ...
    try:
        ...
        logger.openreplica_logger.error(f'Config file not found: {e}')
        return None
    except toml.TomlDecodeError as e:
        logger.openreplica_logger.error(
    ...
    logger.openreplica_logger.debug(f'Loading from toml failed for {agent_config_arg}')
    return None
...
def get_llm_config_arg(
    ...
    logger.openreplica_logger.debug(f'Loading llm config from {llm_config_arg}')
    ...
    logger.openreplica_logger.debug(f'Loading from toml failed for {llm_config_arg}')
    return None
...
def get_condenser_config_arg(
    ...
    try:
        ...
        logger.openreplica_logger.error(f'Config file not found: {toml_file}. Error: {e}')
        return None
    except toml.TomlDecodeError as e:
        logger.openreplica_logger.error(
    ...
    try:
        ...
        logger.openreplica_logger.info(
...
def parse_arguments() -> argparse.Namespace:
    ...
    if args.version:
        print(f'OpenReplica version: {__version__}')
        sys.exit(0)

    return args


def register_custom_agents(config: OpenReplicaConfig) -> None:
...
def setup_config_from_args(args: argparse.Namespace) -> OpenReplicaConfig:
    """Load config from toml and override with command line arguments.

    Common setup used by both CLI and main.py entry points.
    """
    # Load base config from toml and env vars
    config = load_openreplica_config(config_file=args.config_file)
