...
openreplica_logger = logging.getLogger('openreplica')
current_log_level = logging.INFO

if LOG_LEVEL in logging.getLevelNamesMapping():
    current_log_level = logging.getLevelNamesMapping()[LOG_LEVEL]
openreplica_logger.setLevel(current_log_level)

if DEBUG:
    openreplica_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    LOG_TO_FILE = True
    openreplica_logger.debug('DEBUG mode enabled.')

if LOG_JSON:
    openreplica_logger.addHandler(json_log_handler(current_log_level))
else:
    openreplica_logger.addHandler(get_console_handler(current_log_level))

openreplica_logger.addFilter(SensitiveDataFilter(openreplica_logger.name))
openreplica_logger.propagate = False
...
if LOG_TO_FILE:
    openreplica_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    openreplica_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        openreplica_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class OpenReplicaLoggerAdapter(logging.LoggerAdapter):
    extra: dict

    def __init__(
        self, logger: logging.Logger = openreplica_logger, extra: dict | None = None
    ) -> None:
        self.logger = logger
        self.extra = extra or {}
